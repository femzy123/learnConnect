...
export default async function AdminRequestDetailPage({ params }) {
  ...
  const { data: proposals = [] } = await supabase
    ...
    .select("id, teacher_id, status, fee_amount, duration_minutes, created_at, updated_at")
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 space-y-6">
      ...
      <Card>
        <CardContent className="p-6 grid gap-2 text-sm">
          <div><span className="text-muted-foreground">Created:</span> <strong>{new Date(r.created_at).toLocaleString()}</strong></div>
          <div><span className="text-muted-foreground">Category:</span> <strong>{cat?.name || "—"}</strong></div>
          <div><span className="text-muted-foreground">Subject:</span> <strong>{sub?.name || "—"}</strong></div>
          <div><span className="text-muted-foreground">Topic:</span> <strong>{r.topic}</strong></div>
          <div><span className="text-muted-foreground">Status:</span> <strong className="capitalize">{r.status}</strong></div>
        </CardContent>
      </Card>
            ...
            {proposals.map(p => {
              ...
              return (
                <tr key={p.id} className="border-t">
                  ...
                  <td className="px-4 py-3">{p.fee_amount != null ? `₦${(Number(p.fee_amount)/100).toLocaleString()}` : "—"}</td>
                  <td className="px-4 py-3">{p.duration_minutes ? `${p.duration_minutes} mins` : "—"}</td>
                  <td className="px-4 py-3">{new Date(p.updated_at || p.created_at).toLocaleString()}</td>
                </tr>
              );
            })}
            {!proposals.length && (
